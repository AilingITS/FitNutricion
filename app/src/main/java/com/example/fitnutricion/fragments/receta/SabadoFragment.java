...
public class SabadoFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        recyclerView = vista.findViewById(R.id.receta_sabado_List);

        mAuth = FirebaseAuth.getInstance();
        userID = mAuth.getCurrentUser().getUid();
        dbRef = FirebaseDatabase.getInstance().getReference().child("users").child(userID).child("recetas").child("Sabado");
        recyclerView.setHasFixedSize(true);
        recyclerView.setLayoutManager(new LinearLayoutManager(getContext()));

        list = new ArrayList<>();
        myAdapter = new sabadoAdapter(getContext(),list);
        ...
        dbRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull @NotNull DataSnapshot snapshot) {
                ...
                for(DataSnapshot dataSnapshot : snapshot.getChildren()){
                    Sabado foods = dataSnapshot.getValue(Sabado.class);
                    list.add(foods);
                }
