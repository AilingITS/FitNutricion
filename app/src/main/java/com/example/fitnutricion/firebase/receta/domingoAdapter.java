...
public class domingoAdapter extends RecyclerView.Adapter<domingoAdapter.foodsHolder> {

    Context context;
    ArrayList<Domingo> list;

    public domingoAdapter(Context context, ArrayList<Domingo> list) {
    ...
    @NotNull
    @Override
    public domingoAdapter.foodsHolder onCreateViewHolder(@NonNull @NotNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(context).inflate(R.layout.item_list_comidas,parent,false);
        return new domingoAdapter.foodsHolder(v);
    }

    @Override
    public void onBindViewHolder(@NonNull @NotNull domingoAdapter.foodsHolder holder, int position) {
        Domingo foods = list.get(position);
