...
public class juevesAdapter extends RecyclerView.Adapter<juevesAdapter.foodsHolder> {

    Context context;
    ArrayList<Jueves> list;

    public juevesAdapter(Context context, ArrayList<Jueves> list) {
    ...
    @NotNull
    @Override
    public juevesAdapter.foodsHolder onCreateViewHolder(@NonNull @NotNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(context).inflate(R.layout.item_list_comidas,parent,false);
        return new juevesAdapter.foodsHolder(v);
    }

    @Override
    public void onBindViewHolder(@NonNull @NotNull juevesAdapter.foodsHolder holder, int position) {
        Jueves foods = list.get(position);
