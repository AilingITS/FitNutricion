...
import com.example.fitnutricion.fragments.EditarCenaFragment;
...
public class cenaAdapter extends RecyclerView.Adapter<cenaAdapter.foodsHolder> {
    ...
    public cenaAdapter(Context context, ArrayList<Cena> list) {
    ...
    @NotNull
    @Override
    public cenaAdapter.foodsHolder onCreateViewHolder(@NonNull @NotNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(context).inflate(R.layout.item_list_comidas,parent,false);
        return new cenaAdapter.foodsHolder(v);
    }

    @Override
    public void onBindViewHolder(@NonNull @NotNull cenaAdapter.foodsHolder holder, int position) {
        mAuth = FirebaseAuth.getInstance();
        userID = mAuth.getCurrentUser().getUid();

        dbRef = FirebaseDatabase.getInstance().getReference("users").child(userID).child("cenas");

        Cena foods = list.get(position);
                ...
                dialogo1.setItems(opciones, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        if(opciones[which] == "Editar"){
                            ...
                            Fragment myFragment = new EditarCenaFragment(foods.getP_ID());
