...
public class viernesAdapter extends RecyclerView.Adapter<viernesAdapter.foodsHolder> {

    Context context;
    ArrayList<Viernes> list;

    public viernesAdapter(Context context, ArrayList<Viernes> list) {
    ...
    @NotNull
    @Override
    public viernesAdapter.foodsHolder onCreateViewHolder(@NonNull @NotNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(context).inflate(R.layout.item_list_comidas,parent,false);
        return new viernesAdapter.foodsHolder(v);
    }

    @Override
    public void onBindViewHolder(@NonNull @NotNull viernesAdapter.foodsHolder holder, int position) {
        Viernes foods = list.get(position);
