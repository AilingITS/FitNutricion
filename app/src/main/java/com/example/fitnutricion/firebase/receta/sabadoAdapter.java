...
public class sabadoAdapter extends RecyclerView.Adapter<sabadoAdapter.foodsHolder> {

    Context context;
    ArrayList<Sabado> list;

    public sabadoAdapter(Context context, ArrayList<Sabado> list) {
    ...
    @NotNull
    @Override
    public sabadoAdapter.foodsHolder onCreateViewHolder(@NonNull @NotNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(context).inflate(R.layout.item_list_comidas,parent,false);
        return new sabadoAdapter.foodsHolder(v);
    }

    @Override
    public void onBindViewHolder(@NonNull @NotNull sabadoAdapter.foodsHolder holder, int position) {
        Sabado foods = list.get(position);
